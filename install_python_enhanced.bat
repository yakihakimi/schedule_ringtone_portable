@echo off
REM Rules applied
REM Enhanced Python installation script with comprehensive error handling and proxy support
REM This script downloads and installs Python 3.12.4 with multiple fallback options

echo ========================================
echo Enhanced Python Installation
echo ========================================
echo.
...
echo [INFO] Attempting download with PowerShell (Method 1)...
powershell -Command "& {[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { Invoke-WebRequest -Uri '%PYTHON_URL%' -OutFile '%INSTALLER_PATH%' -UseBasicParsing -TimeoutSec 300 } catch { Write-Host 'Method 1 failed, trying with proxy...'; [System.Net.WebRequest]::DefaultWebProxy = [System.Net.WebRequest]::GetSystemWebProxy(); [System.Net.WebRequest]::DefaultWebProxy.Credentials = [System.Net.CredentialCache]::DefaultCredentials; Invoke-WebRequest -Uri '%PYTHON_URL%' -OutFile '%INSTALLER_PATH%' -UseBasicParsing -TimeoutSec 300 } }"

if not exist "%INSTALLER_PATH%" (
    echo [INFO] PowerShell download failed. Trying with curl (Method 2)...
    curl -L -o "%INSTALLER_PATH%" "%PYTHON_URL%" --connect-timeout 30 --max-time 300 --retry 3
)

if not exist "%INSTALLER_PATH%" (
    echo [INFO] Curl download failed. Trying with PowerShell and proxy (Method 3)...
    powershell -Command "& {[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; [System.Net.WebRequest]::DefaultWebProxy = [System.Net.WebRequest]::GetSystemWebProxy(); [System.Net.WebRequest]::DefaultWebProxy.Credentials = [System.Net.CredentialCache]::DefaultCredentials; Invoke-WebRequest -Uri '%PYTHON_URL%' -OutFile '%INSTALLER_PATH%' -UseBasicParsing -TimeoutSec 300 }"
)
...
echo.
echo ========================================
echo [INFO] Offline Installation Options
echo ========================================
echo.

REM Check if we have a local Python installer
set LOCAL_INSTALLER_PATH=python-installer-local.exe
if exist "%LOCAL_INSTALLER_PATH%" (
    echo [INFO] Found local Python installer: %LOCAL_INSTALLER_PATH%
    echo [INFO] Attempting to install from local file...
    echo.
    
    REM Install Python from local file
    echo [INFO] Installing Python from local installer...
    echo [INFO] This may take a few minutes. Please wait...
    echo [INFO] Installation will be silent (no user interaction required)
    echo.
    
    "%LOCAL_INSTALLER_PATH%" /quiet InstallAllUsers=1 PrependPath=1 Include_pip=1 Include_test=0 Include_launcher=1 Include_doc=0 Include_tcltk=1
    ...
    if %errorlevel% equ 0 (
        echo.
        echo ========================================
        echo [SUCCESS] Python installation completed from local file!
        echo ========================================
        echo.
        echo [INFO] Python version:
        python --version
        ...
        echo [INFO] The local installer may be corrupted or incompatible.
        echo.
    )
)

echo [INFO] No local installer found or installation failed.
echo [INFO] Please follow these steps for offline installation:
echo.
echo [INFO] 1. On a computer with internet access, download Python 3.12.4:
echo [INFO]    https://www.python.org/ftp/python/3.12.4/python-3.12.4-amd64.exe
echo.
echo [INFO] 2. Copy the downloaded file to this computer
echo [INFO] 3. Rename it to: python-installer-local.exe
echo [INFO] 4. Place it in the same folder as this script
echo [INFO] 5. Run this script again
echo.
echo [INFO] Alternative: Manual installation
echo [INFO] 1. Download Python 3.12.4 from: https://www.python.org/downloads/
echo [INFO] 2. Run the installer manually
echo [INFO] 3. Make sure to check "Add Python to PATH" during installation
echo [INFO] 4. Restart your terminal and run this script again
echo.
echo [INFO] Recommended version: Python 3.12.4 (64-bit)
echo.
